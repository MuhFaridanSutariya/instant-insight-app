...
def generate_gpt_response(gpt_input, max_tokens):
    ...
    chat = ChatOpenAI(model='gpt-3.5-turbo-0613',
...
def calculate_peer_performance(data):
    # Extract relevant data
    peer_esg_scores = data['esgChart']['result'][0]['peerSeries']['esgScore']
    peer_governance_scores = data['esgChart']['result'][0]['peerSeries']['governanceScore']
    peer_environment_scores = data['esgChart']['result'][0]['peerSeries']['environmentScore']
    peer_social_scores = data['esgChart']['result'][0]['peerSeries']['socialScore']

    company_esg_scores = data['esgChart']['result'][0]['symbolSeries']['esgScore']
    company_governance_scores = data['esgChart']['result'][0]['symbolSeries']['governanceScore']
    company_environment_scores = data['esgChart']['result'][0]['symbolSeries']['environmentScore']
    company_social_scores = data['esgChart']['result'][0]['symbolSeries']['socialScore']

    # Calculate peer average scores
    peer_avg_esg_scores = pd.Series(peer_esg_scores).mean()
    peer_avg_governance_scores = pd.Series(peer_governance_scores).mean()
    peer_avg_environment_scores = pd.Series(peer_environment_scores).mean()
    peer_avg_social_scores = pd.Series(peer_social_scores).mean()

    # Filter out None values from company scores
    filtered_company_esg_scores = [score for score in company_esg_scores if score is not None]
    filtered_company_governance_scores = [score for score in company_governance_scores if score is not None]
    filtered_company_environment_scores = [score for score in company_environment_scores if score is not None]
    filtered_company_social_scores = [score for score in company_social_scores if score is not None]

    # Calculate performance scores
    peer_esg_performance = [peer_avg_esg_scores - score for score in filtered_company_esg_scores]
    peer_governance_performance = [peer_avg_governance_scores - score for score in filtered_company_governance_scores]
    peer_environment_performance = [peer_avg_environment_scores - score for score in filtered_company_environment_scores]
    peer_social_performance = [peer_avg_social_scores - score for score in filtered_company_social_scores]

    df = pd.DataFrame({
        'PeerEsgScorePerformance': peer_esg_performance,
        'PeerGovernancePerformance': peer_governance_performance,
        'PeerEnvironmentPerformance': peer_environment_performance,
        'PeerSocialPerformance': peer_social_performance
    })
...
if submit and response_df is not None:
    with ui_container:
        # symbol = Ticker('Kbr')
        # first_key = next(iter(symbol.summary_profile))
        # employees = symbol.summary_profile[first_key]['fullTimeEmployees']
        # st.write("{:,}".format(employees))
        # first_key = next(iter(symbol.price))
        # st.write(symbol.price[first_key]['shortName'])
        # cola_y = yf.Ticker('Kbr')
        # esg_scores = symbol.asset_profile
        # st.write(cola_y.esg_scores)
        ...
        # get stock info
        key = next(iter(ticker.price))
        name = ticker.price[key]['shortName']

        # sector_key = next(iter(ticker.summary_profile))
        sector = ticker.summary_profile[key]['sector']

        # industry_key = next(iter(ticker.summary_profile))
        industry = ticker.summary_profile[key]['industry']

        # employees_key = next(iter(ticker.summary_profile))
        employees = ticker.summary_profile[key]['fullTimeEmployees']

        country = ticker.summary_profile[key]['country']
        city = ticker.summary_profile[key]['city']
        website = ticker.summary_profile[key]['website']
        summary = ticker.summary_profile[key]['longBusinessSummary']
        logo_url = 'https://logo.clearbit.com/' + website
        ...
        if requests.get(logo_url).status_code == 200:
            ...
            try:
                url = "https://query2.finance.yahoo.com/v1/finance/esgChart?symbol=FTNT"
                
                connection = urllib.request.urlopen(url)
                raw = connection.read()
                data_json = json.loads(raw)
                data = {
                    'esgScore': data_json['esgChart']['result'][0]['symbolSeries']['esgScore'],
                    'governanceScore': data_json['esgChart']['result'][0]['symbolSeries']['governanceScore'],
                    'environmentScore': data_json['esgChart']['result'][0]['symbolSeries']['environmentScore'],
                    'socialScore': data_json['esgChart']['result'][0]['symbolSeries']['socialScore']
                }
                ...
                result = calculate_peer_performance(data_json)
                # get esg score data for the company
                total_esg = df["esgScore"].sum()
                governance_score = df['governanceScore'].sum()
                environment_score = df['environmentScore'].sum()
                social_score = df['socialScore'].sum()

                result = calculate_peer_performance(data_json)

                # get peer group data
                peers_total_esg_avg = result['PeerEsgScorePerformance'].mean()
                peers_governance_avg = result['PeerGovernancePerformance'].mean()
                peers_env_avg = result['PeerEnvironmentPerformance'].mean()
                peers_social_avg = result['PeerSocialPerformance'].mean()

                nama = data_json['esgChart']['result'][0]['peerGroup']

                esg_dict = {'Type': [nama, 'Peer Group'],
                ...
                pivot_df = esg_df_melted.pivot(index='variable', columns='Type', values='value')

                pivot_df.plot(kind='bar', figsize=(10, 6))
                plt.title('ESG Scores Comparison')
                plt.xlabel('ESG Score Type')
                plt.ylabel('Score Value')
                plt.xticks(rotation=45, ha='right')
                plt.legend(title='Type')
                plt.tight_layout()

                plt.savefig('esg.png', bbox_inches='tight', pad_inches=0)
...
elif submit and response_df.empty:
    with ui_container:
